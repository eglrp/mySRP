...
void GPolygon::computeBoundingBox()
{
    ...
    for( int i = 0 ; i< points.size(); i++ )
    {
        ...
        //updating the bounding box of local system which will get the AABB of this geometry in local system
        
        boundingBox.updateBoundingBoxLimits(points[i], boundingBox.minLimit_l, boundingBox.maxLimit_l);
    }
    
    //generate the bounding box contains 8 verices, than convert all of them to BFS and update the limits to get the AABB in BFS
    GVector box[8];
...
/*
 ...
 //    See: Algorithm 1 "Area of Triangles and Polygons", http://geomalgorithms.com/a01-_area.html
...
bool GPolygon::PnPoly( gfc::GVector& p )
{
    ...
    // after this projection, all the points should have 0 z coordinate on the projection plane
    // applying the winding number algorithm
...
/*
 ...
 首先计算出与平面的交点，在判断点是否在多变形内部，采用拓扑学中的回转数概念(winding number algorithm in topology)
 平面方程是： (x-x0)Tn = 0
 */
bool GPolygon::intersection( GRay &ray, gfc::GVector &intersection, gfc::GVector &normal )
{
    bool ret = false;
    // arbitrary vertex
    ...
    //Ntn is used to check the direction of ray, if it is from the back side of the polygon
