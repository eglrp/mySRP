...
/*
 the basic idea is convert the ray into local system, find the solution in local system
 and then convert back to BFS
 */
bool GCone::intersection(GRay &ray, gfc::GVector &intersection, gfc::GVector &normal)
{
    bool ret = false;
    ...
    //because the cone is a lathegeometry, the axis equation is basically: z=a*x+b
    // radius bottom has to be larger than top radius, this equation is proper for both cone and truncated cone
    double b = radius_t*height/(radius_b - radius_t) + height;
    double a = -b/radius_b;
    
    
    //check if the start of the ray is on the cone?
    double zz = a*sqrt(ray_origin_local.x*ray_origin_local.x + ray_origin_local.y*ray_origin_local.y)+b;
    if(fabs(zz-ray_origin_local.z)<GGeometry::EPS) // ray starts from cone, return false
    {
        return false;
    }
    //the equation for the cone is: z= a*sqrt(x^2 + y^2) + b, 0<=z<=h;
    // the equation for the ray is: x = s + d*t, d is the direction vector, s is the origin of the ray, t is the parameter
    //this will generate a quadratic equation with parameter t
    double sz_b = ray_origin_local.z - b;
    double A = ray_dir_local.z*ray_dir_local.z - a*a*(ray_dir_local.x*ray_dir_local.x + ray_dir_local.y*ray_dir_local.y);
    double B = 2.0*(  sz_b*ray_dir_local.z - a*a*(ray_origin_local.x*ray_dir_local.x + ray_origin_local.y*ray_dir_local.y  ) );
    double C = sz_b*sz_b - a*a*(ray_origin_local.x*ray_origin_local.x + ray_origin_local.y*ray_origin_local.y );
    
    
    double t=0.0,t0 = 0.0, t1 =0.0;
    if( fabs(A) <= GGeometry::EPS )  // which means the ray is paraller to the cone surface(no intersection)
    {
        return false;
    }
    else
    ...
    //first check the sign of the solution, if they both satisfy the requirements, then choose the closest one
    // z-b should be the same sign with a for the correct cone, the other solution may be the other virtual cone, since its a bi-cone structure
        ...
        )
    {
        return false;
            ...
            &&z_b1 * a > 0.0)  // choose the closest point
    {
    ...
    double temp = sqrt(candidate_local.x*candidate_local.x + candidate_local.y*candidate_local.y);
