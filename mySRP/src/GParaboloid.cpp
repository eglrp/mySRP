...
bool GParaboloid::intersection(GRay &ray, gfc::GVector &intersection,
                               gfc::GVector &normal) {
  bool ret = false;
  ...
  double B = -(2.0 * ray_origin_local.x * ray_dir_local.x +
               2.0 * ray_origin_local.y * ray_dir_local.y +
               r * r / h * ray_dir_local.z);
  double C = r * r - ray_origin_local.x * ray_origin_local.x -
             ray_origin_local.y * ray_origin_local.y -
             r * r / h * ray_origin_local.z;

  double t = 0.0, t0 = 0.0, t1 = 0.0;

  if (fabs(A) <
      GGeometry::EPS)  // the ray is parallerl to the z axis, only one solution
  {
    t = -C / B;
  } else {
