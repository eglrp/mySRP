...
bool GSphere::intersection(GRay &ray, GVector& intersection, gfc::GVector &normal)
{
    GVector ret ;
...
//    t = t0;
    ...
    if( fabs(ptp- radius_squared)< GGeometry::EPS )  // the ray is just on this sphere
    {
        return false;
    }
    
    double dtd = ray.direction.norm2(); // should always be 1.0, since direction is unit vector
    double ptd = p.x*ray.direction.x + p.y*ray.direction.y + p.z*ray.direction.z;
    ...
    // if t1 <0 , that means it is in the opposite direction of the ray which could happen when the ray start is inside a sphere
