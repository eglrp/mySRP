...
/*
 ...
 the basic idea is to convet the ray into local system, get the solution in
 local system and convert back to BFS

*/

bool GCylinder::intersection(GRay &ray, gfc::GVector &intersection,
                             gfc::GVector &normal) {
  bool ret = false;
  ...
  double A =
      ray_dir_local.x * ray_dir_local.x + ray_dir_local.y * ray_dir_local.y;
  double B = 2.0 * (ray_origin_local.x * ray_dir_local.x +
                    ray_origin_local.y * ray_dir_local.y);
  double C = ray_origin_local.x * ray_origin_local.x +
             ray_origin_local.y * ray_origin_local.y - radius * radius;

  double t = 0.0, t0 = 0.0, t1 = 0.0;

  if (A == 0.0)  // don't conside the cap of the cylinder,  the ray is basically
                 // along z axis
  {
    return false;
  } else if (A != 0.0) {
