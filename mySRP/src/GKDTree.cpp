...
void KDNode::split(std::vector<std::unique_ptr<GGeometry>>& geo,
  ...
  for (int i = 0; i < geo.size(); i++) {
    ...
    // have to call std::move, the original object will be deleted aftering
    // calling move

    // for every object, it has only two choice, left or right
    switch (splitAxis) {
      case 0:
        splitPos > center.x ? myright.push_back(std::move(geo[i]))
                            : myleft.push_back(std::move(geo[i]));
        break;
      case 1:
        splitPos > center.y ? myright.push_back(std::move(geo[i]))
                            : myleft.push_back(std::move(geo[i]));
        break;
      case 2:
        splitPos > center.z ? myright.push_back(std::move(geo[i]))
                            : myleft.push_back(std::move(geo[i]));
        break;
...
void KDNode::insert(std::vector<std::unique_ptr<GGeometry>>& geo, int& depth) {
  ...
  for (int i = 0; i < geo.size(); i++) {
    // update the minimum
    aabb.updateBoundingBoxLimits(geo[i]->boundingBox.minLimit, aabb.minLimit,
                                 aabb.maxLimit);
    // update the maximum
    aabb.updateBoundingBoxLimits(geo[i]->boundingBox.maxLimit, aabb.minLimit,
                                 aabb.maxLimit);
  }
  ...
  if (node->aabb.intersect(ray, dis))  // hits the AABB of the whole spacecraft
  {
    ...
    if (node->leaf == true)  // we have reached a leaf
    {
      for (int i = 0; i < node->geometryList.size(); i++) {
        bool test =
            node->geometryList[i]->intersection(ray, myintersection, mynormal);
...
// if it is true, the specularity, reflectivty and intersection information
// would be updated
...
int KDTree::getTreeDepth() { return Depth(root); }
