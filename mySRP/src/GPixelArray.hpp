...
// the origin of the solar array is defined at geometric center of the solar
// array assuming all the rays from the pixel array are parallel, same
// direction, different start point
class GPixelArray {
 public:
  ...
  static double
      d0;  // the distance from pixel array to the spacecraft model origin

  // the two directions of pixel array, different for different ray
  // this is really important when the origin of BFS is not at the geometry
  // centre
  GVector dx;
  GVector dy;
