...
bool GRing::intersection(GRay &ray, gfc::GVector &intersection,
                         gfc::GVector &normal) {
  bool ret = false;
  GVector x0 = center;
  ...
  double Ntn =
      ray.direction.x * cz.x + ray.direction.y * cz.y + ray.direction.z * cz.z;
  ...
  if (dis <= radius_external && dis >= radius_internal) {
