...
void GScene::computeBoundingBox() {
  ...
  // because GObjects are in BFS, that means the local bounding box is the same
  // as BFS bounding box
  cs.set(0, 0, 0);
  cx.set(1, 0, 0);
  cy.set(0, 1, 0);
  ...
  for (int i = 0; i < components.size(); i++) {
    ...
    boundingBox.updateBoundingBoxLimits(p, boundingBox.minLimit_l,
                                        boundingBox.maxLimit_l);
    GVector& q = components[i]->boundingBox.maxLimit;

    boundingBox.updateBoundingBoxLimits(q, boundingBox.minLimit_l,
                                        boundingBox.maxLimit_l);
  }

  // because all the components in this scene are in BFS
  // there is no need to convert back, get the AABB of this scene in
  // boundingBox;
  GVector box[8];
...
void GScene::loadUserfile(std::string filename) {
  ...
  // this should be set according to the userfile, the emissivity(infrared) for
  // MLI is 0.84
  ...
  while (infile) {
    ...
    if ((tmp = strstr(line, "\n"))) *tmp = '\0';
    //去掉回车符
    if ((tmp = strstr(line, "\r"))) *tmp = '\0';

    if ((tmp = strstr(line2, "\n"))) *tmp = '\0';
    //去掉回车符
    if ((tmp = strstr(line2, "\r"))) *tmp = '\0';

    // check number of items scanned from sscanf return value
    sscanf(line2, "%d %s %d %d", &record_num, temp, &mli_type,
    ...
    if (!strcmp(line, "GENERAL")) {
      ...
      for (int count = 0; count < num; count++) {
        infile >> general->vertices[count].x >> general->vertices[count].y >>
            general->vertices[count].z;
      }
      ...
      infile >> cylinder->vertices[0].x >> cylinder->vertices[0].y >>
          cylinder->vertices[0].z;
      infile >> cylinder->vertices[1].x >> cylinder->vertices[1].y >>
          cylinder->vertices[1].z;
      ...
      infile >> cirlce->vertices[0].x >> cirlce->vertices[0].y >>
          cirlce->vertices[0].z;
      infile >> cirlce->vertices[1].x >> cirlce->vertices[1].y >>
          cirlce->vertices[1].z;
      ...
      infile >> ring->vertices[0].x >> ring->vertices[0].y >>
          ring->vertices[0].z;
      infile >> ring->vertices[1].x >> ring->vertices[1].y >>
          ring->vertices[1].z;
      ...
      infile >> cone->depth;  // the distance from the bottom to the top
                              // vertex(which is a virtual point in this case)
      infile >> cone->radius_b;
      infile >> cone->height;

      // calculate the radius of the top
      cone->radius_t =
          cone->radius_b * (cone->depth - cone->height) / cone->depth;
      ...
      infile >> paraboloid->center_b.x >> paraboloid->center_b.y >>
          paraboloid->center_b.z;
      infile >> paraboloid->center_t.x >> paraboloid->center_t.y >>
          paraboloid->center_t.z;

      infile >> paraboloid->op.solar_reflectivity >>
          paraboloid->op.solar_specularity;
      ...
      paraboloid->op.solar_absorptivity =
          1.0 - paraboloid->op.solar_reflectivity;
      ...
      // calculate the radius of the top
      paraboloid->radius_t = (paraboloid->depth - paraboloid->height) *
                             paraboloid->radius_b * paraboloid->radius_b /
                             paraboloid->depth;
  ...
  // update the bounding box of GObjects itself, find the AABB for the whole
  // satellite
  // the AABB for the whole satellite will be used to determine the size of the
  // pixel array
  computeBoundingBox();
  ...
  // this can not be done in the ray tracer, because ray tracer only process the
  // parts that are illuminated by the sun
