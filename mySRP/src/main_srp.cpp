...
int main(int argc, const char* argv[]) {
...
#endif
  // int depth = 0;
  // myraytracer.kdtree.showTree(myraytracer.kdtree.root,0,depth );

  double t0 = clock();

  // printf("lat lon ray_intersected totalRay ratio area_hit pixelarray_x
  ...
  for (int j = 0; j < lon_vec.size(); j++) {
    ...
    for (int i = 0; i < lat_vec.size(); i++) {
      ...
      //            sprintf(buffer,"%.3f %.3f %ld %lu %f %f %.3f %.3f %.16E
      //            %.16E %.16E %.16E\n",
      //                    lat, lon,myraytracer.rayIntersected
      ...
      /*
      fprintf(output,"%.3f %.3f %ld %lu %f %f %.3f %.3f  %.16E  %.16E %.16E
      %.16E\n", lat, lon,myraytracer.rayIntersected
      ,myraytracer.pixelarray.totalRay,
                      double(myraytracer.rayIntersected)/myraytracer.pixelarray.totalRay,
                      myraytracer.area_hit,
