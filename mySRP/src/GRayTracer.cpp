...
double GRayTracer::T0 =
    2.7 * 2.7 * 2.7 * 2.7;  // the temperature of the outer space is 2.7 Kelvin
double GRayTracer::emissivity_eff = 0.02;  // the infrared effective emissivity,
                                           // should be read from configure file

// this value is specific for TRR calculation, and it is a constant need to be
// set in the configure file
double GRayTracer::bus_inside_energy = GRayTracer::emissivity_eff *
                                       GRayTracer::sigma * GRayTracer::T4 /
                                       GRayTracer::SOLAR_CONST;

double GRayTracer::MLI_emissive_heat = GRayTracer::sigma *
                                       GRayTracer::emissivity_eff *
                                       (GRayTracer::T4 - GRayTracer::T0);

// after calling this function, the objects.component is invalide
void GRayTracer::makeTree() { kdtree.buildKDTree(objects.components); }
...
void GRayTracer::SRP_cov(GRay& ray, GVector& normal,
...
GVector GRayTracer::SRP_t(GRay& ray, GVector& normal,
  ...
  // the diffuse reflected radiation force + the absorbed radiation is emmited
  // as thermal energy immediantly
  GVector f3 =
      -2.0 / 3.0 * factor * (1.0 - reflectivity * specularity) * normal;
...
void GRayTracer::SRP(GVector& force, GRay& ray, GVector& normal,
                     GVector& reflectionDirection, double specularity,
                     double reflectivity) {
  double factor = ray.energy;
  ...
  GVector f3 =
      -2.0 / 3.0 * factor * reflectivity * (1.0 - specularity) * normal;
 ...
 */
GVector GRayTracer::processor(GRay& ray, GVector& normal,
                              GVector& reflectionDirection,
...
SRP_cov(ray, normal, reflectionDirection, op.solar_specularity,
  ...
  } else  // thermal response for the other kind of material, including
          // radiation and conduction, no convection in space assume all the
          // absorbed radiation is emmited as thermal energy
  {
    TRR_regular(trr, ray, normal, reflectionDirection, op.solar_specularity,
                op.solar_reflectivity);
  }
...
/*
 ...
 assuming that the absorpted energy will be emited in the form of heat
 immediantly this is suitable for the material that with very low heat capacity
 and completely prevent heat transfer toward the satellite interior,like MLI
 this assumption is different from the one used by UCL
 */
void GRayTracer::TRR_regular(GVector& force, GRay& ray, GVector& normal,
                             GVector& reflectionDirection, double specularity,
                             double reflectivity) {
  double factor = ray.energy;
...
/*
 This TRR response function only account for the effect from the solar
 radiaiton, ignore the heat radiated from the internal satellite bus, because
 this effect has been account for when building the spacecraft geometry without
 ray tracing

 */
void GRayTracer::TRR_MLI2(GRay& ray, GVector& normal, double emissivity,
                          double abosrbtivity, GVector& force) {
  double factor = ray.energy;
  double cos_theta =
      -dotproduct(ray.direction, normal) / normal.norm() / ray.direction.norm();
  double mag = 0.0;

  // when cos_theta is near 0, which means ray is perpendicular to the surface
  // normal, the force should be zero
  if (cos_theta >= GGeometry::EPS) {
    mag = 2.0 / 3.0 * emissivity *
          (factor * abosrbtivity + GRayTracer::bus_inside_energy / cos_theta) /
          (emissivity + GRayTracer::emissivity_eff);

    // for temperature test
    // double t4 = 1368.0*(abosrbtivity +
    // GRayTracer::bus_inside_energy)/(emissivity +
    // GRayTracer::emissivity_eff)/GRayTracer::sigma; double mag2
    // = 2.0/3.0*GRayTracer::sigma*emissivity*t4/299792458.0*GPixelArray::pixelArea;
  }
...
/*
 the function to calculate the thermal re-radiation pressure for the satellite
 bus MLI
 */
void GRayTracer::TRR_MLI(GRay& ray, GVector& normal, double emissivity,
                         double abosrbtivity, GVector& force) {
  double factor = ray.energy;
  double cos_theta =
      -dotproduct(ray.direction, normal) / normal.norm() / ray.direction.norm();
 ...
 */
int GRayTracer::intersect(GRay& ray, GVector& intersection, GVector& normal,
                          GOpticalProperty& op) {
  int objectIndex = -1;
  bool ret = false;
  ...
  for (int i = 0; i < objects.components.size(); i++) {
    /*
    double volumn = (objects.components[i]->boundingBox.maxLimit.x -
    objects.components[i]->boundingBox.minLimit.x)
                    *(objects.components[i]->boundingBox.maxLimit.y -
    objects.components[i]->boundingBox.minLimit.y)
                    *(objects.components[i]->boundingBox.maxLimit.z -
    objects.components[i]->boundingBox.minLimit.z);
    ...
    */
    ...
    if (ret == true)  // if intersects, need to find the closed intersection
                      // point to ray source
    {
...
void GRayTracer::raytracing(GRay& ray, GVector& force, double cov[9]) {
  ...
  if (ray.reflectionNum >= GRayTracer::MAX_DEPTH) {
    ...
    else  // intersection point found
    {
      ...
      // reflectDirection is a unit vector, do not have to be normalised, make
      // it faster
      reflectDirection =
          -2.0 * dotproduct(ray.direction, normal) * normal + ray.direction;
      ...
      ray.inv_direction.set(1.0 / reflectDirection.x, 1.0 / reflectDirection.y,
                            1.0 / reflectDirection.z);
      ...
      // the enegy reduced after reflection, this is only the energy of specular
      // refelction
...
GVector GRayTracer::run() {
  ...
  // these are the first, second and third row of the covariance matrix of the
  // final force
  double c[9] = {0.0};
  ...
  //        intersectionPoints[i].x, intersectionPoints[i].y,
  //        intersectionPoints[i].z);
  //    }
