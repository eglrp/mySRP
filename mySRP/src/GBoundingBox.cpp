...
void GBoundingBox::computeCenter()
{
    center = (minLimit + maxLimit)/2.0;
}

//update min and max according to p
// so p is the input while min and max are outputs
void GBoundingBox::updateBoundingBoxLimits(gfc::GVector& p, gfc::GVector& min, gfc::GVector& max)
{
    if( p.x <=  min.x )
    {
        min.x = p.x;
...
// ref http://gamedev.stackexchange.com/questions/18436/most-efficient-aabb-vs-ray-collision-algorithms
...
//    dis = 0.0; // the length of the ray, this is useful to check the first intersection point
//    
//    double t1 = (minLimit.x - ray.start.x)*ray.inv_direction.x;
//    double t2 = (maxLimit.x - ray.start.x)*ray.inv_direction.x;
