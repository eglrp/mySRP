...
/*
 ...
 steps:
 1. rotate aroud z with angle lambda, make BFS x in the projection of ray on BFS
 xoy plane
 2. rotate aroud y with angle 3pi/2 - phi, make BFS z axis pointing to the ray
 direction(W).
...
void GPixelArray::buildUVW() {
  ...
  s.set(GPixelArray::d0 * cos(lat) * cos(lon),
        GPixelArray::d0 * cos(lat) * sin(lon), GPixelArray::d0 * sin(lat));
}
...
void GPixelArray::buildPixelArray(double Lat, double Lon, GBoundingBox& aabb) {
  ...
  GVector minLimit(GBoundingBox::infinite, GBoundingBox::infinite,
                   GBoundingBox::infinite);  // in UVW system
  GVector maxLimit(-GBoundingBox::infinite, -GBoundingBox::infinite,
                   -GBoundingBox::infinite);  // in UVW system
  ...
  GVector refpoint[4] = {
      GVector(minLimit.x, minLimit.y, 0), GVector(maxLimit.x, minLimit.y, 0),
      GVector(minLimit.x, maxLimit.y, 0),
      GVector(maxLimit.x, maxLimit.y,
              0)};  // refpoint[0]-->minimum u, refpoint[1]-->minimum v,
                    // refpoint[2]-->maximum v

  // two directions of the rectangle pixel array in UVW, dx is perpendicular to
  // dy
  ...
  firstPixel = refpoint[0] - 0.5 * GPixelArray::pixelSize * dx -
               0.5 * GPixelArray::pixelSize * dy;
  ...
  //calculate the minus u and minus v pixel position which is the minimum
  coordinate firstPixel.set(-(num_x-1)/2.0*GPixelArray::pixelSize,
  -(num_y-1)/2.0*GPixelArray::pixelSize, 0.0); firstPixel += pixelArrayCenter;
  */
...
bool GPixelArray::getRay(GRay& ray) {
  ...
  GVector pixelPos;
  pixelPos = firstPixel + c_i * GPixelArray::pixelSize * dx +
             c_j * GPixelArray::pixelSize * dy;
...
void GPixelArray::generateRays() {
  ...
  for (int i = 0; i <= num_x; i++) {
    for (int j = 0; j <= num_y; j++) {
      pixelPos = firstPixel + i * GPixelArray::pixelSize * dx +
                 j * GPixelArray::pixelSize * dy;
